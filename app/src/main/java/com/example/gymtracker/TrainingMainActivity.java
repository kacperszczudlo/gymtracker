...
public class TrainingMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton.setOnClickListener(v -> {
            ...
            // Sprawdź, czy log istnieje, jeśli nie, utwórz go
            boolean logExists = dbHelper.trainingLogExists(userId, currentSelectedDateString, selectedDayName);
            if (!logExists) {
                boolean createdFromPlan = dbHelper.createEmptyTrainingLogFromPlan(userId, selectedDayName, currentSelectedDateString);
                if (!createdFromPlan) {
                    long id = dbHelper.createEmptyTrainingLog(userId, selectedDayName, currentSelectedDateString);
                    Log.d("DEBUG_LOG", "Brak planu – stworzono pusty log (" + currentSelectedDateString + ", " + selectedDayName + "), id=" + id);
                }
            }
            // Zapisz dane
